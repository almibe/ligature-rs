...
fn tokenize_integer() {
    let input = "123450";
    let res: Vec<Token> = tokenize_and_filter(input).iter().map(|t| t.first().unwrap().0.clone()).collect();
...
fn tokenize_integers() {
    ...
    let res: Vec<_> = tokenize_and_filter(input).unwrap().iter().map(|t| t.0.clone()).collect();
...
fn tokenize_strings() {
    ...
    let res = tokenize_and_filter(input).unwrap().first().unwrap().0.clone();
